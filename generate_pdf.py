...
def create_pdf():
    ...
    styles.add(ParagraphStyle(name='Center', alignment=1, fontSize=16, spaceAfter=10, fontName="Helvetica-Bold"))
    styles.add(ParagraphStyle(name='Heading1_Custom', fontSize=14, spaceBefore=15, spaceAfter=10, fontName="Helvetica-Bold", textColor=colors.HexColor("#2E75B6")))
    styles.add(ParagraphStyle(name='Heading2_Custom', fontSize=12, spaceBefore=10, spaceAfter=5, fontName="Helvetica-Bold", textColor=colors.HexColor("#2E75B6")))
    styles.add(ParagraphStyle(name='Normal_Custom', fontSize=11, spaceAfter=8, fontName="Helvetica"))
    styles.add(ParagraphStyle(name='Code_Custom', fontSize=9, fontName="Courier", spaceAfter=8, leftIndent=10, textColor=colors.HexColor("#333333")))
    ...
    data_header = [
        ...
        ["Trabalho", "Processamento Distribuído - Parte B (Paralelismo)"],
        ["Grupo", "Alfredo Neto, Carlos, Cláudio Pontes, Dgeison Serrão Peixoto, Eduardo Hosda, Francisco Duclou"],
        ["Data", "28 de abril de 2026"],
        ["GitHub", "https://github.com/cllaud99/trabalho-paralelismo-parteb"]
    ]
    t_header = Table(data_header, colWidths=[3*cm, 13*cm])
    ...
    elements.append(Paragraph("As métricas a seguir foram obtidas a partir do processamento completo do arquivo yellow_tripdata_2016-02.csv (11.382.049 registros). Os resultados foram produzidos pelas versões sequencial (Parte A) e paralela (Parte B), com valores idênticos em ambas, confirmando a corretude da implementação paralela.", styles['Normal_Custom']))
    ...
    t_metrics = Table(data_metrics, colWidths=[1*cm, 11*cm, 4*cm])
    ...
    elements.append(Paragraph("Obs.: o valor mínimo negativo (-$ 450,30) é um dado real do dataset, correspondente a estornos e disputas registrados pela NYC TLC.", ParagraphStyle(name='Small', fontSize=9, fontName="Helvetica-Oblique")))
    ...
    t_payment = Table(data_payment, colWidths=[2*cm, 7*cm, 4*cm, 3*cm])
    ...
    t_days = Table(data_days, colWidths=[3*cm, 8*cm, 5*cm])
    ...
    t_perf = Table(data_perf, colWidths=[3*cm, 8*cm, 3*cm, 2*cm])
    t_perf.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.HexColor("#D5E8F0")),
        ('TEXTCOLOR', (0,0), (-1,0), colors.black),
        ('TEXTCOLOR', (3,2), (3,2), colors.red), # Highlight speedup
        ('ALIGN', (0,0), (-1,-1), 'LEFT'),
        ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'),
        ('FONTNAME', (0,1), (-1,-1), 'Helvetica'),
        ('FONTSIZE', (0,0), (-1,-1), 10),
    ...
    log_seq = """[00:34:45] Iniciando Parte A — versão sequencial<br/>[00:34:46] Chunk 10 processado | Corridas acumuladas: 1,000,000 | Tempo: 1.0s<br/>[00:34:56] Chunk 110 processado | Corridas acumuladas: 11,000,000 | Tempo: 10.8s<br/>Leitura concluída. 114 chunks processados.<br/>Tempo total de execução: 11.18 segundos"""
    elements.append(Paragraph(log_seq, styles['Code']))

    elements.append(Paragraph("5.2 Parte B — Versão Paralela", styles['Heading2_Custom']))
    log_par = """[00:34:57] Iniciando Parte B — versão paralela<br/>Workers: 28 | Chunk: 100000<br/>[00:35:01] Chunks recebidos: 10 | Corridas: 1,000,000 | Tempo: 4.0s<br/>[00:35:10] Chunks recebidos: 110 | Corridas: 11,000,000 | Tempo: 13.5s<br/>Concluído. 114 chunks processados.<br/>Tempo paralelo de execução: 13.85 segundos"""
    elements.append(Paragraph(log_par, styles['Code']))
    ...
    data_tech = [
        ...
        ["Bibliotecas", "pandas 3.0.2, numpy 2.4.4, multiprocessing"],
        ["Paralelismo", "multiprocessing.Pool com imap_unordered"],
        ["Ambiente", "Linux (WSL2), 28 CPUs lógicas"],
        ["Dataset", "yellow_tripdata_2016-02.csv (11.3M registros, ~2GB)"]
    ]
    ...
    print("Documento final gerado com sucesso: Relatorio_Trabalho_Paralelismo_ParteB_Final.pdf")
